...
- `openMessage(id)`: dispatches a real click event on the corresponding row element (so Zimbra's own UI actually opens it, exactly as if the user clicked it), waits for the reading pane to render, then scrapes subject/from/date/body HTML back out.
- `startCompose()` / `fillCompose(to, subject, body)` / `clickSend()`: clicks the real Compose button, waits for the compose form to appear, sets the to/subject/body fields (setting `.value` and dispatching `input`/`change` events so Zimbra's own JS framework picks up the change, since a raw property assignment alone often won't trigger framework-bound state), then dispatches a real click on the actual Send button.
...
- **Selector discovery is unknown until tested live**: Zimbra's DOM structure (class names, ARIA labels, whether it's classic Zimbra or the newer "Zimbra X" web client) can only be determined by inspecting the real, authenticated `webmail.iitb.ac.in` page — this plan can't guess exact selectors blind. First implementation pass will need a live session (DevTools open on the real site) to identify robust selectors — preferring stable attributes (`aria-label`, `data-*` ids) or Zimbra's documented keyboard shortcuts (e.g. `c` for compose) over fragile generated class names, since keyboard-shortcut-driven automation tends to survive Zimbra UI updates better than CSS selectors.
- `observer.js`: a `MutationObserver` on the message-list container, posting `{type: "newMail", ...}` back via `chrome.webview.postMessage` whenever Zimbra's own UI inserts a new row — this is what drives live refresh and notifications, sourced directly from the real UI changing rather than a polling timer.
...
- Minimalistic visual language: flat panels, generous whitespace, one accent color, system font — deliberately not mimicking Zimbra's own UI.
...
- Mark-read/delete from the custom UI; confirm the change reflects in webmail.iitb.ac.in when checked in a real browser (proves the click really landed on Zimbra's own controls, not just local state).
...
## Known open risk
Exact DOM structure/selectors of the live Zimbra client can only be determined by inspecting the real, authenticated `webmail.iitb.ac.in` page — not guessable in advance. Phase B's first implementation pass will need one or more live-testing iterations (with DevTools open against the real site) to nail down working selectors/event-dispatch details, and this automation approach is inherently more fragile than an API-based integration: if IITB updates Zimbra's web UI, the injected JS selectors may need updating.
